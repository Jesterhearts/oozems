...
mod tests {
    ...
    #[test]
    fn local_npc_act_archive_evidence_is_consistent() {
        let data = Path::new(env!("CARGO_MANIFEST_DIR")).join("../../data");
        let quest_path = data.join("Quest.wz");
        let npc_path = data.join("Npc.wz");
        if !quest_path.exists() || !npc_path.exists() {
            return;
        }

        let quest_root = super::open_archive(&quest_path).expect("quest archive");
        super::parse(&quest_root, "quest archive root".to_owned()).expect("parse quest archive");
        let actions = super::child(&quest_root, "Act.img")
            .expect("Act.img lookup")
            .expect("Act.img");
        let checks = super::child(&quest_root, "Check.img")
            .expect("Check.img lookup")
            .expect("Check.img");
        let info = super::child(&quest_root, "QuestInfo.img")
            .expect("QuestInfo.img lookup")
            .expect("QuestInfo.img");
        for (name, node) in [
            ("Act.img", &actions),
            ("Check.img", &checks),
            ("QuestInfo.img", &info),
        ] {
            super::parse(node, format!("quest archive {name}")).expect("parse quest image");
        }

        let npc_root = super::open_archive(&npc_path).expect("NPC archive");
        super::parse(&npc_root, "NPC archive root".to_owned()).expect("parse NPC archive");
        let content = super::NpcContent::open_optional(&data)
            .expect("open NPC content")
            .expect("NPC content");
        for (quest_id, expected_action, expected_npc_id) in [
            ...
            let quest_name = quest_id.to_string();
            let action = super::child(&actions, &quest_name)
                .expect("quest action lookup")
                .expect("quest action");
            let completion_action = super::child(&action, "1")
                .expect("completion action lookup")
                .expect("completion action");
            assert_eq!(
                super::string_value(&completion_action, "npcAct").expect("npcAct value"),
                Some(expected_action.to_owned()),
                "quest {quest_id} npcAct",
            );

            let check = super::child(&checks, &quest_name)
                .expect("quest check lookup")
                .expect("quest check");
            let completion_check = super::child(&check, "1")
                .expect("completion check lookup")
                .expect("completion check");
            let npc_id = super::int_value(&completion_check, "npc")
                .expect("completion NPC value")
                .and_then(|value| u32::try_from(value).ok())
                .expect("positive completion NPC");
            assert_eq!(npc_id, expected_npc_id, "quest {quest_id} completion NPC");

            let quest_info = super::child(&info, &quest_name)
                .expect("quest info lookup")
                .expect("quest info");
            let auto_complete = super::int_value(&quest_info, "autoComplete")
                .expect("autoComplete value")
                .unwrap_or_default();
            let auto_pre_complete = super::int_value(&quest_info, "autoPreComplete")
                .expect("autoPreComplete value")
                .unwrap_or_default();

            let npc = super::child(&npc_root, &format!("{npc_id:07}.img"))
                .expect("NPC lookup")
                .expect("completion NPC definition");
            super::parse(&npc, format!("NPC {npc_id}")).expect("parse completion NPC");
            let linked_id = linked_definition_id(&npc).expect("NPC link");
            assert_eq!(linked_id, None, "quest {quest_id} NPC link");
            let source = match linked_id.filter(|linked_id| *linked_id != npc_id) {
                Some(linked_id) => {
                    let linked = super::child(&npc_root, &format!("{linked_id:07}.img"))
                        .expect("linked NPC lookup")
                        .expect("linked NPC definition");
                    super::parse(&linked, format!("linked NPC {linked_id}"))
                        .expect("parse linked NPC");
                    linked
                }
                None => npc,
            };
            let animation_names = super::sorted_children(&source)
                .expect("NPC fields")
                .into_iter()
                .filter_map(|animation| {
                    let name = super::node_name(&animation).ok()?;
                    let has_frames =
                        super::sorted_children(&animation)
                            .ok()?
                            .into_iter()
                            .any(|frame| {
                                super::node_name(&frame)
                                    .is_ok_and(|frame_name| frame_name.parse::<u32>().is_ok())
                            });
                    has_frames.then_some(name)
                })
                .collect::<Vec<_>>();
            assert!(
                animation_names.iter().any(|name| name == expected_action),
                "quest {quest_id} NPC {npc_id} does not have animation {expected_action:?}",
            );
            assert_eq!(auto_complete, 0, "quest {quest_id} must not auto-complete");
            assert_eq!(
                auto_pre_complete, 0,
                "quest {quest_id} must not auto-pre-complete",
            );
            ...
            let asset_ids = definition
                .assets
                .iter()
                .map(|asset| asset.id.as_str())
                .collect::<std::collections::HashSet<_>>();
            assert_eq!(asset_ids.len(), definition.assets.len());
            assert!(definition.animations.iter().all(|animation| {
                animation.frames.iter().all(|frame| {
                    asset_ids.contains(frame.asset_id.as_str())
                        && content.get_asset(&frame.asset_id).is_some()
                })
            }));
        }

        let linked_pair = super::sorted_children(&npc_root)
            .expect("NPC definitions")
            .into_iter()
            .find_map(|npc| {
                let source_id = super::node_name(&npc)
                    .ok()?
                    .strip_suffix(".img")?
                    .parse::<u32>()
                    .ok()?;
                super::parse(&npc, format!("NPC {source_id}")).ok()?;
                let target_id = linked_definition_id(&npc).ok()??;
                (target_id != source_id).then_some((source_id, target_id))
            })
            .expect("at least one linked NPC definition");
        let source = content
            .get_definition(linked_pair.0)
            .expect("load linked source NPC")
            .expect("linked source NPC definition");
        let target = content
