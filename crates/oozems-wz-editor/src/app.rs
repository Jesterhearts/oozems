    ...
    match app.tab {
        ...
        EditorTab::Skills => {
            if let Some(document) = app.skill_document.as_mut() {
                draw_definition_document(ui, document, true, &mut app.skill_add_drafts);
...
fn draw_quest_editor(
    ...
    match app.quest_view.clone() {
        QuestView::Definition => {
            ...
            draw_definition_document(ui, document, false, &mut app.skill_add_drafts);
...
fn draw_node(
    ...
    if !node.accepts_properties() {
        return draw_scalar(ui, node, skill_id, structural && !root);
    }
    let level = node
        .path
        .rsplit_once('/')
    ...
    let label = if root {
        ...
        let summary = skill_level_summary(node, skill_id.unwrap_or_default());
        if summary.is_empty() {
...
fn skill_level_summary(
    ...
    level
        ...
        .filter_map(|property| {
            ...
            let label = skill_property_label(skill_id, &property.name);
...
fn draw_scalar(
    ...
    ui.horizontal(|ui| {
        ...
        let property_label = skill_id
            .map(|skill_id| skill_property_label(skill_id, &node.name))
...
fn skill_property_label(
    ...
    let semantic = match (skill_id, name) {
        (4_000_000, "x") => Some("Accuracy"),
        (4_000_000, "y") => Some("Avoidability"),
        (_, "hpCon") => Some("HP cost"),
        (_, "mpCon") => Some("MP cost"),
        (_, "hp") => Some("HP"),
        (_, "mp") => Some("MP"),
        (_, "pad") => Some("Weapon attack"),
        (_, "mad") => Some("Magic attack"),
        (_, "acc") => Some("Accuracy"),
        (_, "eva") => Some("Avoidability"),
        (_, "pdd") => Some("Weapon defense"),
        (_, "mdd") => Some("Magic defense"),
        (_, "speed") => Some("Speed"),
        (_, "jump") => Some("Jump"),
        (_, "str") => Some("Strength"),
        (_, "damage") => Some("Damage"),
        (_, "fixdamage") => Some("Fixed damage"),
        (_, "criticalDamage") => Some("Critical damage"),
        (_, "mastery") => Some("Mastery"),
        (_, "attackCount") => Some("Attack count"),
        (_, "mobCount") => Some("Target count"),
        (_, "time") => Some("Duration"),
        (_, "cooltime") => Some("Cooldown"),
        (_, "hs") => Some("Level description selector"),
        (_, "range") => Some("Range"),
        (_, "prop") => Some("Success chance"),
        _ => None,
    };
    semantic.map_or_else(
        || name.to_owned(),
        |semantic| format!("{semantic} ({name})"),
    )
}
...
fn save_active(app: &mut EditorApp) {
    ...
    let result = match app.tab {
        ...
        EditorTab::Skills => {
            ...
            if app.skill_staged == app.skill_saved
                && app.skill_structure == app.skill_structure_saved
            {
                Err(anyhow::anyhow!("there are no unsaved skill changes"))
            } else if edits.is_empty() {
                crate::save::write_archive_source(&app.paths.skill, &app.paths.skill_output).map(
                    |_| {
                        format!(
                ...
                crate::save::write_archive_property_edits(
                    &app.paths.skill,
...
mod tests {
    ...
    fn skill_property_labels_keep_raw_nodes_and_explain_affected_stats() {
        assert_eq!(skill_property_label(1_002, "speed"), "Speed (speed)");
        assert_eq!(
            skill_property_label(1_002, "hs"),
            "Level description selector (hs)"
        );
        assert_eq!(skill_property_label(4_000_000, "x"), "Accuracy (x)");
        assert_eq!(skill_property_label(4_000_000, "y"), "Avoidability (y)");
        assert_eq!(skill_property_label(1, "unknown"), "unknown");
    }
    ...
    fn live_skill_summary_includes_added_effects_and_hides_hs_metadata() {
        ...
        assert_eq!(
            skill_level_summary(&level, 1_002),
